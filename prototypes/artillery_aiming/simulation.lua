...
function Simulation.run(cmd)
    ...
    if State.bombard.useStoredCharge and ctx.hasCharge then
        chargeOverride = State.bombard.chargeLevel * 4.0
    end

    local vOffset, fForwardVelocity, fUpwardVelocity, vCollisionMax, fGravity, fDrag =
        Entity.GetProjectileInformation(
            pWeapon, ctx.isDucking, ctx.itemCase, ctx.itemDefIndex,
            ctx.weaponID, pLocal, chargeOverride
        )
    ...
    local results = traceHull(
        ...
        vCollisionMin, vCollisionMax, TRACE_MASK
    )
    if results.fraction ~= 1 then return end
    vStartPosition = results.endpos

    if ctx.itemCase == -1
        or ((ctx.itemCase >= 7 and ctx.itemCase < 11) and fForwardVelocity ~= 0)
    then
    ...
    if ctx.itemCase == -1 then
        results = traceHull(
            vStartPosition, vStartPosition + (vStartAngle:Forward() * 10000),
            vCollisionMin, vCollisionMax, TRACE_MASK
        )
