...
local g_iPolygonTexture = draw.CreateTextureRGBA(
    "\xff\xff\xff" .. string.char(Config.visual.polygon.a), 1, 1
)
...
local function drawImpactPolygon(plane, origin)
    if not Config.visual.polygon.enabled then return end
...
function Visuals.drawTrajectory()
    ...
    if not traj.isValid then return end
...
function Visuals.drawAimGuide()
    ...
    if not st.originPoint or not st.targetPoint then return end

    local start2d = worldToScreen(st.originPoint)
    local end2d = worldToScreen(st.targetPoint)
    if not start2d or not end2d then return end
