...
local function tryPlayerMove(origin, velocity, mins, maxs, index, tickinterval)
	...
	for bumpcount = 0, 3 do
		...
		local trace = engine.TraceHull(origin, end_pos, mins, maxs, GameConstants.MASK_PLAYERSOLID, function(ent, contentsMask)
			return ent:GetIndex() ~= index
		end)
...
function PlayerTick.simulateTick(playerCtx, simCtx)
	...
	if is_on_ground then
		...
		airAccelerate(playerCtx.velocity, wishdir, playerCtx.maxspeed, simCtx.sv_airaccelerate, tickinterval, 0, playerCtx.entity)
		playerCtx.velocity.z = playerCtx.velocity.z - GameConstants.DEFAULT_GRAVITY * tickinterval
	end
	
	playerCtx.origin = tryPlayerMove(playerCtx.origin, playerCtx.velocity, playerCtx.mins, playerCtx.maxs, playerCtx.index, tickinterval)
